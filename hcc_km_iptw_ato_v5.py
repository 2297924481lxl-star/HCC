...
def draw_risk_table(ax_table, d_no, d_yes, times):
    ...
    no_counts = [
        float(coerce_num(d_no.loc[d_no["time60"] >= t, "_weight"]).sum())
        for t in times
    ]
    yes_counts = [
        float(coerce_num(d_yes.loc[d_yes["time60"] >= t, "_weight"]).sum())
        for t in times
    ]

    header = "Time: " + "  ".join([f"{int(t):>5d}" for t in times])
    row_no = "No  : " + "  ".join([f"{c:>5.1f}" for c in no_counts])
    row_yes = "Yes : " + "  ".join([f"{c:>5.1f}" for c in yes_counts])

    ax_table.text(0.01, 0.78, header, fontsize=9, family="monospace", color=COL_GRAY)
    ax_table.text(0.01, 0.42, row_no, fontsize=9, family="monospace", color=COL_NO)
    ax_table.text(0.01, 0.08, row_yes, fontsize=9, family="monospace", color=COL_IMMUNE)

    ax_table.text(
        0.01, -0.18,
        "Number at risk represents sum of weights rather than actual patient counts.",
        fontsize=8, color=COL_GRAY, transform=ax_table.transAxes
    )
...
class KMCompareApp(QMainWindow):
    ...
    def plot_one_panel(self, ax_km, ax_tbl, d_panel: pd.DataFrame, title: str):
        ...
        km_no.fit(
            d_no["time60"].values,
            event_observed=d_no["status60"].values,
            weights=coerce_num(d_no["_weight"]).values,
            label="No immunotherapy"
        )
        km_yes.fit(
            d_yes["time60"].values,
            event_observed=d_yes["status60"].values,
